...
def generate_pdf_file(request, pk):
    ...
    p.drawRightString(540, 390, text_converter("ماده4 : تعهدات طرفین:"))
    p.drawRightString(540, 370, text_converter("۴-۱: میزان ساعت هر جلسه برای استفاده ی متقاضي ۹۰ دقيقه"))
    p.drawRightString(540, 350, text_converter("آموزشی امکان پذیر نیست مگر با مجوز کتبی داده شده توسط مجموعه و حداکثر نفرات استفاده کننده"))
    p.drawRightString(540, 330, text_converter("از سوي متقاضی در هر سانس اعلام باید گردد."))
    p.drawRightString(540, 290, text_converter("۴-۲: متقاضی متعهد مي شود تا نهایت امانت داری را در طي مدت قرارداد رعایت فرماید و مبلغ"))
    p.drawRightString(540, 270, text_converter("هرگونه خسارت به تجهیزات و تاسیسات ورزشی را پرداخت کند."))
    p.drawRightString(540, 250, text_converter("۴-۳: مجموعه ورزشی حجاب میتواند در صورت نیاز به سالن، با اعلام حداقل دو هفته زودتر،"))
    p.drawRightString(540, 230, text_converter("ساعات مورد نیاز خود جابه جا یا قرارداد را به طور کلي لغو کند که در صورت لغو قرارداد، وجوه"))
    p.drawRightString(540, 210, text_converter("واريزي بابت سانس هاي استفاده نشده مسترد خواهدشد."))
    p.drawRightString(540, 190, text_converter("۴-۴ : تعطيلي تمرينات به علت شرایط خاص آن، منوط به نظر مجموعه ورزشی حجاب خواهد بود که"))
    p.drawRightString(540, 170, text_converter("در سالن، جلسات جبرانی در اختیار تیم متقاضي قرار خواهد گرفت."))
    p.showPage()
    p.save()
    buffer.seek(0)
