...
def verify(request):
    ...
    if response['Status'] == 100:
        ...
        sliced_queryset = Time.objects.filter(day__name=reservation.time.day.name, time=reservation.time.time,
                                              day__month__number__gt=reservation.time.day.month.number).filter(day__month__number=reservation.time.day.month.number,day__number__gte=reservation.time.day.number)[:reservation.session]
